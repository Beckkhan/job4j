...
public class UserTest {
     ...
     *
     * Такая же ситуация складывается в случае, если мы переопределяем только метод equals.
     * Хэш-коды теперь становятся не равны, соответственно, ключи не воспринимаются как идентичные.
     *
     * Пример наглядно демонстрирует подтверждение правила, которое говорит о том,
     * что при переопределении метода hashCode обязательно надо переопределить метод equals.
     * Равенство хэш-кодов не означает идентичность обектов, но если выполнение метода equals
     * возвращает true, в этом случае и хэш-коды равны.
     */
    @Test
